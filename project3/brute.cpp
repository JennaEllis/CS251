#include <cstdlib>
#include <fstream>
#include <iostream>
#include <string>


#include "key.hpp"
#include "brute.hpp"
#include "math.h"

std::string me;
std::string encrypted;
std::string table_filename;
bool verbose = false;

Brute::Brute(const std::string& filename) {
	T.resize(N);
	std::string buffer;
    std::fstream input(filename.c_str(), std::ios::in);
    for (int i = 0; i < N; i++) {
        std::getline(input, buffer);
        T[i].set_string(buffer);
    }
    input.close();
}




void Brute::decrypt(const std::string& encrypted){
	// your code here
	unsigned long long int lim = (long) pow(2, N); //all password combinations

	char letters[C+1];
	char init[C+1];

	//initialize values for keys
	for(int index = 0; index < C; index++){
			if(index == C-1){
				letters[index] = ALPHABET[1];
			}
			else{
				letters[index] = ALPHABET[0];
			}
			init[index] = ALPHABET[0];
	}

	Key add(letters); //"aaaaaa....b"
	Key current(init); //"aaaaaaa..."

	Key encr(encrypted);

	for( unsigned long long int i = 0; i < lim; i++){
			if(current.subset_sum(T,verbose) == encr){
				current.show_word(); //fix da print	
			}
			current+=add; //increment current password
	}

}

void usage(const std::string& error_msg="") {
	if (!error_msg.empty()) std::cout << "ERROR: " << error_msg << '\n';
	std::cout << me << ": Brute force cracking of Subset-sum password with " 
		<< B << " bits precision\n"
	    << "USAGE: " << me << " <encrypted> <table file> [options]\n"
		<< "\nArguments:\n"
		<< " <encrypted>:   encrypted password to crack\n"
		<< " <table file>:  name of file containing the table to use\n"
		<< "\nOptions:\n"
		<< " -h|--help:     print this message\n"
		<< " -v|--verbose:  select verbose mode\n\n";
	exit(0);
}

void initialize(int argc, char* argv[]) {
	me = argv[0];
	if (argc < 3) usage("Missing arguments");
	encrypted = argv[1];
	table_filename = argv[2];
	for (int i=3; i<argc; ++i) {
		std::string arg = argv[i];
		if (arg == "-h" || arg == "--help") usage();
		else if (arg == "-v" || arg == "--verbose") verbose = true;
		else usage("Unrecognized argument: " + arg);
	}
}


int main(int argc, char *argv[]){
	
	initialize(argc, argv);
	Brute b(table_filename);
	//clock_t start = clock();
	// insert your code here
	b.decrypt(encrypted);
	//clock_t end = clock();

	//double elapsed = double(end-start) / CLOCKS_PER_SEC;
	//std::cout<< elapsed << '\n';
	return 0;
}